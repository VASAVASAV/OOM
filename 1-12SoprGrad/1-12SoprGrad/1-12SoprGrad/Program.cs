...
namespace _1_8Huk
{
    class MyProg : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.Name = "MyProg";
            this.Text = "Метод спряжених градієнтів";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            double TempLambda;
            double[] LastDir = new double[NumberOfDim];
            for (i = 0; i < NumberOfDim; i++)
            {
            ...
            LastDir = Grad;
            double beta;
            Grad = FindGrad(pars, function, TempPoint);
            double[] LastDitTemp = new double[NumberOfDim];
            while (FindLeng(Grad) > epsilon)
            {
                ...
                for (i = 0; i < NumberOfDim; i++)
                {
                    beta = (Scalar(Grad, Grad) / Scalar(LastGrad, LastGrad));
                    if (Math.Abs(beta - 1) < 0.001)
                        beta -= -0.001;

                    //TempPoint[i] = CurPoint[i] - LastLambda * Math.Sign(Grad[i]);
                    TempPoint[i] = CurPoint[i] + LastLambda * (-1*Grad[i]+beta*LastDir[i]);
                }
                /////
                for (i = 0; i < NumberOfDim; i++)
                {
                ...
                while (NextFunc >= CurFunc)
                {
                    ...
                    for (i = 0; i < NumberOfDim; i++)
                    {
                        ...
                        if (Math.Abs(beta - 1) < 0.001)
                            beta -= -0.001;
                        //TempPoint[i] = CurPoint[i] - LastLambda * Math.Sign(Grad[i]);
                        //TempPoint[i] = CurPoint[i] + LastLambda * (-1*Grad[i]);
                        TempPoint[i] = CurPoint[i] + LastLambda * (-1 * Grad[i] + beta * LastDir[i]);
                    }
                    /////
                    for (i = 0; i < NumberOfDim; i++)
                ...
                for (i = 0; i < NumberOfDim; i++)
                {
                    ...
                    if (Math.Abs(beta - 1) < 0.001)
                        beta -= -0.001;
                    CurPoint[i] += LastLambda * (-1 * Grad[i] + LastDir[i] * beta);
                    LastDitTemp[i] = (-1 * Grad[i] + LastDir[i] * beta);
                }
                for (i = 0; i < NumberOfDim; i++)
                {
                    LastDir[i] =LastDitTemp[i];
                }
                /////
                for (i = 0; i < NumberOfDim; i++)
                {
