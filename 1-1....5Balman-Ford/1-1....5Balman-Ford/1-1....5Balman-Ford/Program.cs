...
namespace _1_1._._._._4Ford_Falcerson
{
    class MyForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
                ...
                for (i = 0; i < NumOfVec; i++)
                {
                    for (j = 0; j < NumOfVec; j++)
                    {
                        if (dataGridView1.Rows[i].Cells[j].Value == "")
                            dataGridView1.Rows[i].Cells[j].Value = "" +int.MaxValue;
            ...
            int Temp;
            int [] Answers = new int[dataGridView1.Rows.Count];
            ...
            // while (MoveMade)
             {
                ...
                while (StepMade)
                {
                    ...
                    {
                        if (LastWays[i].CheckCycle())
                        {
                            continue;
                        }
                        if (LastWays[i].Length >= Answers[LastWays[i].CurWay[LastWays[i].CurWay.Count-1]])
                        {
                            continue;
                        }
                        Answers[LastWays[i].CurWay[LastWays[i].CurWay.Count-1]] = LastWays[i].Length;
                        DeleteNeeded = true;
                ...
                /*LastWays.Sort();
                if (LastWays.Count != 0 && (LastWays[LastWays.Count - 1].Length != 0 || LastWays[LastWays.Count - 1].Length != int.MaxValue))
                {
                    AllStream += LastWays[LastWays.Count - 1].Length;
                    for (i = 1; i < LastWays[LastWays.Count - 1].CurWay.Count; i++)
                    {
                        dataGridView1.Rows[LastWays[LastWays.Count - 1].CurWay[i - 1]].Cells[LastWays[LastWays.Count - 1].CurWay[i]].Value = Convert.ToInt32(dataGridView1.Rows[LastWays[LastWays.Count - 1].CurWay[i - 1]].Cells[LastWays[LastWays.Count - 1].CurWay[i]].Value) - LastWays[LastWays.Count - 1].Length;
                        dataGridView1.Rows[LastWays[LastWays.Count - 1].CurWay[i]].Cells[LastWays[LastWays.Count - 1].CurWay[i - 1]].Value = Convert.ToInt32(dataGridView1.Rows[LastWays[LastWays.Count - 1].CurWay[i]].Cells[LastWays[LastWays.Count - 1].CurWay[i - 1]].Value) + LastWays[LastWays.Count - 1].Length;
                    }
                    MoveMade = true;
                }*/
            }
            ...
            {
                textBox3.Text += "Vertex #" + (i + 1) + " - " + Answers[i] + Environment.NewLine;
            }
    ...
    class Way : IEquatable<Way>, IComparable<Way>
    {
        ...
        public int CompareTo(Way compareWay)
        {
            ...
            if (compareWay == null)
                ...
                return this.Length.CompareTo(compareWay.Length);//this.PartId.CompareTo(comparePart.PartId);
        }
